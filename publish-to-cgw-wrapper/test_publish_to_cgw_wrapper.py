...
def test_call_cgw_api_success(mock_request):
    ...
    response = cgw_wrapper.call_cgw_api(
        "https://cgw.com/cgw/rest/admin", "GET", "/endpoint", None
    )
...
@patch("requests.request")
def test_call_cgw_api_failure(mock_request):
    ...
    with pytest.raises(RuntimeError, match="API call failed: Unauthorized"):
        cgw_wrapper.call_cgw_api("https://cgw.com/cgw/rest/admin", "GET", "/endpoint", None)


@patch("requests.request")
def test_call_cgw_api_exception(mock_request):
...
def test_get_product_id(mock_call):
    ...
    product_id = cgw_wrapper.get_product_id(
        "https://cgw.com/cgw/rest/admin", None, "product_name_1", "code1"
    )
    mock_call.assert_called_once_with(
        "https://cgw.com/cgw/rest/admin", "GET", "/products", None
    )
...
def test_get_product_id_not_found(mock_call):
    ...
    with pytest.raises(
        ...
        cgw_wrapper.get_product_id(
            "https://cgw.com/cgw/rest/admin", None, "product_name_3", "code3"
        )
    mock_call.assert_called_once_with(
        "https://cgw.com/cgw/rest/admin", "GET", "/products", None
    )
...
def test_get_version_id(mock_call):
    ...
    version_id = cgw_wrapper.get_version_id(
        "https://cgw.com/cgw/rest/admin", None, "4010426", "1.2"
    )
    mock_call.assert_called_once_with(
        "https://cgw.com/cgw/rest/admin", "GET", "/products/4010426/versions", None
    )
...
def test_get_version_id_not_found(mock_call):
    ...
    with pytest.raises(ValueError, match="Version not found: 1.3"):
        cgw_wrapper.get_version_id("https://cgw.com/cgw/rest/admin", None, "4010426", "1.3")
    mock_call.assert_called_once_with(
        "https://cgw.com/cgw/rest/admin", "GET", "/products/4010426/versions", None
    )
...
def test_generate_metadata(content_dir, data_file, metadata):
    ...
    output_metadata = cgw_wrapper.generate_metadata(
        str(content_dir), components, "product_code_1", 4156067, "1.1", True
    )
...
def test_rollback_files(mock_call):
    ...
    response = cgw_wrapper.rollback_files(
        "https://cgw.com/cgw/rest/admin", None, "101", "201", [4567, 5678]
    )
    mock_call.assert_has_calls(
        [
            call(
                "https://cgw.com/cgw/rest/admin",
                "DELETE",
                "/products/101/versions/201/files/4567",
                None,
            ),
            call(
                "https://cgw.com/cgw/rest/admin",
                "DELETE",
                "/products/101/versions/201/files/5678",
                None,
            ),
        ]
    )
...
def test_create_files_success(mock_call, metadata):
    ...
    created, skipped = cgw_wrapper.create_files(
        "https://cgw.com/cgw/rest/admin", None, "101", "201", metadata
    )
    ...
    mock_call.assert_has_calls(
        [
            call(
                "https://cgw.com/cgw/rest/admin",
                "GET",
                "/products/101/versions/201/files",
                None,
            ),
            call(
                "https://cgw.com/cgw/rest/admin",
...
def test_create_files_with_existing(mock_call, metadata):
    ...
    created, skipped = cgw_wrapper.create_files(
        "https://cgw.com/cgw/rest/admin", None, "101", "202", metadata
    )

    expected_calls = [
        call(
            "https://cgw.com/cgw/rest/admin", "GET", "/products/101/versions/202/files", None
        ),
        call(
            "https://cgw.com/cgw/rest/admin",
            "POST",
            "/products/101/versions/202/files",
            None,
            metadata[2],
        ),
        call(
...
def test_create_files_exception(mock_call, metadata):
    ...
    with pytest.raises(RuntimeError, match="File can not be created"):
        cgw_wrapper.create_files(
            "https://cgw.com/cgw/rest/admin", None, "101", "201", metadata[:2]
        )

    expected_calls = [
        call(
            "https://cgw.com/cgw/rest/admin", "GET", "/products/101/versions/201/files", None
        ),
        call(
            "https://cgw.com/cgw/rest/admin",
            "POST",
